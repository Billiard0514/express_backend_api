...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'uploads/news'); // Directory where files will be stored
    },
...
export const upload = multer({ storage, limits: { fileSize: 100 * 1024 * 1024 } }); // Limit file size to 100MB
...
export const addNews = async (req: Request, res: Response) => {
    ...
    try {
        if (!title || !content || !brand || !email) {
            return res.status(400).json({ message: 'All fields (title, content, brand, email) are required' });
        }
        ...
        const newNews = new NewsModel({
            ...
            brand: brandDoc._id, // Use brandDoc._id instead of brand
